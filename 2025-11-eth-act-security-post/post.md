...
We use the following terms and abbreviations:
 - EL: The Execution Layer in the Ethereum stack (FOOTNOTE: [1]). EL clients are responsible for checking validity of blocks, among other responsiblities.
...
Level of Concern: [deferred to others]

## Security Component: The Guest Program
By the guest program we mean: the STF compiled to a binary that can be executed by a ZKVM.

### Potential Issue: New bugs in STFs and bugs in new STFs 
...
Rust support for [RV32IM is only Tier 2](https://doc.rust-lang.org/rustc/platform-support/riscv32-unknown-none-elf.html), while [RV64GC is Tier 2 "with host tools"](https://doc.rust-lang.org/rustc/platform-support/riscv64gc-unknown-linux-gnu.html). (but this target emits Linux syscalls). An example CI run of Rust tests against a RISC-V 64-bit target is as https://github.com/rust-lang/rust/actions/runs/19116334329/job/54626328406

Go, by contrast, seems to do robust testing of RISC-V. For instance, in on [this dashboard]() we found this [CI failure of a RISC-V target](https://ci.chromium.org/ui/p/golang/builders/ci/gotip-linux-riscv64/b8699006944076070321/overview) where 127 out of 61836 testse fail. Unfortunately, Go offer much less control over the RISC-V code that is emitted, which (at present) can use a rather large set of extensions, Linux syscalls, and does not ofter built-in floating point emulation.
...
[The RISE Project](https://riseproject.dev/) project is pursuing improvements compiler testing and support for RISC-V targets. Links to some blog posts on their work are provided in  

Level of Concern: High; compilers are highly complicated black boxes in this project, any poor testing of these means that ZKVMs can produce binaries that do not share the semantics of the STF under all inputs. It is obvious that bugs can be catastrophic; for a particular example see (REF [3]), and for a nice overview see the final sections of (REF [4]). 
...
Level of Concern: Medium-High; these precompiles tend to be complex. (⛔TODO⛔: links on blst bugs for EVM precompile evidence?)
...
Mitigations: Thorough test suites exist to test compliance of of RISC-V emulators. These are being run the ZKEVM Team's [ZKEVM Test Monitor](https://eth-act.github.io/zkevm-test-monitor/). Compliance testing for additional ISAs should be run nightly and tracked similarly. 
...
Mitigations: Exhaustive unit testing of fundamental circuits is critical. Thorough audits and compelling bug bounties are critical. Fuzzing has been shown to be effective (REF https://arxiv.org/pdf/2411.02077)
...
Level of concern: High, as this is a meta-issue incompassing many others.

Mitigations: All of the EEST test cases should be proven. It may make sense to add ZKEVM-focused failure cases as well, to show that proof verification fails under certain conditions.



### Potential Issue: Transpilation
The ZKVMs transform an input binary to a representation that suitable for proving. In some cases this is quite faithful to the RISC-V itself, while in other cases this introduces another low-level abstraction that a programmer or auditor must understand. In some cases this is done for performance reasons, specifically conerning the emulation of these programs. If the transpilation step has a bug, for instance it silently NOPs a block of instructions, then witness generation for that part of the program is meaningless.
...
(⛔TODO⛔ sanity check)In addition to generating a complete trace for the execution of a client for a given set of inputs and possibly transpiling it to an internal representation suitable for proving, a ZKVM must generate a complete witness for proving. This is a kind of finer-grained execution trace generation that, for instance, may genreate data for lookup arguments or constraints that glueing constraints that link different parts of the program. If the witness generation is incorrect, even for a correct arithmetic circuit, the system might incorrectly reject a valid transaction, or incorrectly accept an invalid transaction.

Level of concern: Medium--this is single component related to other high-impact issues. 

Mitigations: 


### Potential Issue: Protocol Bug in whitepaper
Bugs are found in even the most well-known papers (⛔TODO⛔: examples of PlonK, GKR, others)--in fact, one some be more cautious using protocols described in less studied and newer papers.

Level of concern: Medium

Mitigations: When systems implement systems described in less well-studied papers, implementing teams and their auditors should conduct a review of the paper, possibly seeking guidance and opinions from an expert in the theory.
...
The protocols as written in papers rarely-if-ever match the protocols that teams intend to implement. For instance, batching of polynomial commitments might be added or modified, or added steps that require additional prover-verifier interactions may be incorrectly fused into other steps. Another, less concerning sort of discrepancy is that the papers on SNARKs protocols often fix an arithmetization that is simple, such as R1CS or the 'vanilla' madd gate in PlonK. For production-grade systems capable of proving Ethereum in real time, dramatically more complex setups are needed. In practice, this component is rather modular, and not too much of a concern for security of the protocol (but _is_ a BIG concern for semantic correctness of the ZKEVM!).
...
Mitigations: Precise, clear specifications of the protocol should be written, ideally before implementation. These should be kept current. Auditors should read these specifications and be sure that security proofs for "basic" protocols do in fact carry over under modification.
...
Currently, https://clientdiversity.org/ shows that there are three clients with with over 10% market share, and five with over 1% market share.  If only one or two clients are competitive (on a speed and cost basis) in a world with ZKEVMs, then client diversity has become worse. It should be noted that new clients, such as Ethrex (⛔TODO⛔ others?) are being written. While replacing battle tested clients by new clients would be a hit to security, in the long term a it could improve diversity. 
...
# Questions:
What should level of concern represent? Or should qualitative measure be broken out into more specific pieces? Should I get more opinions?
Does the code for any particular EL go away?
Does the EVMMAX example actually make sense here?

# Notes:
Add somewhere that general testing at ever level "goes without saying".
Have to say the normal caveat that the term ZKVM does not actually mean ZK
Talk about differences in response.
Introduce term "circuit machine"
Introduce term "snarkel"
Be sure to talk about formal verification

# Links for research or potentially to share
[Rise on Go support](https://riseproject.dev/2025/04/04/advancing-go-on-risc-v-progress-through-the-rise-project/)
[RISE on Tier-1 support for RISC-V](https://riseproject.dev/2025/04/15/project-rp004-support-for-a-64-bit-risc-v-linux-port-of-rust-to-tier-1/)
[Rise on LLVM integration](https://riseproject.dev/2024/10/15/working-with-igalia-to-improve-risc-v-llvm-continuous-integration/?utm_source=chatgpt.com)
RISE on RISC-V support

# References:
[1] https://ethereum.org/developers/docs/nodes-and-clients/ 
[2] https://eest.ethereum.org/main/
[3] https://www.certora.com/blog/llvm-bug
[4] https://argument.xyz/blog/riscv-good-bad/
